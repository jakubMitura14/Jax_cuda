...
def window_reverse(windows, window_size, H, W):
    ...
    get patchified image back into original dimensions    
    """
...
def log_space_continuous_position_bias(window_size):
    ...
    so in paper it is explained that it is to deal with the fact that we have diffrent resolutions
    in a pyramid
    hence here as far As I get it we get the matrix that tell us about
    the diffrence in localisation - distance between two points in a window  
    """
...
def create_attn_mask(shift_size, input_resolution, window_size):
    ...
    """
    if shift_size > 0:
        H, W = input_resolution #resolution changes when we progress - as far as I get it the bigger the window the lower the resolution
...
class SwinTransformerBlock(nn.Module):
    ...
    def setup(self):
        if min(self.input_resolution) <= max(self.window_size):
            self.shift_size2 = 0
            #so window size is directly related to input resolution
    ...
    def __call__(self, inputs, *, deterministic):
        ...
        if self.shift_size2 > 0:
            # so instead of moving the indicies of the window we are moving the whole image through roll !!
            shifted_x = jnp.roll(x, shift=(-self.shift_size2, -self.shift_size2), axis=(1, 2))
        else:
            shifted_x = x
        # partition image into tokesn - patches
        ...
        # merge windows important here we have window_size2 not window_size
...
class PatchMerging(nn.Module):
    ...
    so this one as far as I get it is just to combine patches and lower the resolution 
    ...
    def __call__(self, x):
        ...
        x = x.reshape(B, H, W, C)
        #this way we are reucing the resolution of the image but we keep all of the information by increasing number of channels
        ...
        x = self.norm(x)
        x = self.reduction(x)# probably is reducing the number of channels
...
class PatchEmbed(nn.Module):
    ...
    def setup(self):
        #important we calclulate here the resolution !! so resolution means basically how many patches will be in a window
        patches_resolution = [self.img_size[0] // self.patch_size[0], 
                              self.img_size[1] // self.patch_size[1]]
        self.patches_resolution = patches_resolution
        #number of patches per window  
        self.num_patches = patches_resolution[0] * patches_resolution[1]
        #get convolution to use local information
